...
const FEATURE_COST_DISPLAY: { name: string; cost: number }[] = [
  { name: "Análise de currículo", cost: 5 },
  { name: "Criar currículo", cost: 10 },
  { name: "Adaptar currículo", cost: 15 },
  { name: "Carta de apresentação", cost: 10 },
  { name: "Análise de empresa", cost: 5 },
];
...
export default function PacksPage() {
  ...
  const handleConfirmPurchase = async () => {
    ...
    setLoading(true);
    try {
      const { url } = await startCheckout(selectedPack.id);
      // Redireciona pro checkout hospedado do Abacate Pay. As moedas só são
      // creditadas quando o webhook confirmar o pagamento — o user volta pra
      // /plans?payment=success após pagar.
      ...
      <section className="relative overflow-hidden rounded-3xl bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl">
        ...
        <div className="relative p-8 md:p-12 animate-fade-in-up">
          ...
          <p className="text-gray-400 mt-4 text-base md:text-lg max-w-xl leading-relaxed">
            Compre moedas para usar todas as funcionalidades. Compra única via PIX ou cartão, sem assinatura.
          </p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch animate-fade-in-up animation-delay-400">
        {CREDIT_PACKS.map((pack) => (
          <PackCard
            key={pack.id}
            pack={pack}
            isPopular={pack.id === "intermediary"}
        ...
        {selectedPack && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-400">
              Ao confirmar, você será redirecionado para o Abacate Pay (PIX ou
              cartão). Suas moedas são creditadas automaticamente após a
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={loading}
              >
                {loading ? "Abrindo checkout..." : "Pagar agora"}
              </Button>
            </div>
          </div>
        )}
...
function PackCard({
  ...
  return (
    <div
      className={`relative rounded-2xl flex min-w-0 ${
        isPopular
          ? "gradient-border bg-white/[0.03] backdrop-blur-xl pricing-popular !overflow-visible"
          : "bg-white/[0.03] backdrop-blur-xl border border-white/[0.06] overflow-hidden"
      } hover:border-primary-500/30 hover:-translate-y-0.5 transition-all duration-300 group`}
    >
      ...
      {isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
          <span className="px-3 py-1 bg-gradient-to-r from-primary-500 to-accent-violet text-white text-xs font-bold rounded-full glow-blue whitespace-nowrap">
            MAIS POPULAR
          </span>
        </div>
      )}
      <div className="relative p-5 lg:p-7 flex flex-col w-full min-w-0">
        <div>
          ...
          <h3 className="text-lg font-bold text-white font-heading whitespace-nowrap">{pack.name}</h3>
          <div className="flex items-baseline gap-1 mt-2 flex-wrap">
            ...
            <span className="text-gray-500 text-xs">único</span>
          </div>
        ...
        <ul className="space-y-2 mt-4 flex-1">
          {pack.features.map((feature) => (
            <li key={feature} className="flex items-start gap-2 text-sm text-gray-300">
              <div className="bg-gradient-to-br from-primary-500 to-accent-violet rounded-full p-0.5 flex-shrink-0 mt-0.5">
